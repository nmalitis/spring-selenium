...
public class UserVisaTest extends SpringBaseTestNGTest {
    ...
    public void visaTest(User u){
        ...
        logger.info("Request conf number INFO : " + this.registrationPage.getConfirmationNumber());
        logger.warn("Request conf number WARNING : " + this.registrationPage.getConfirmationNumber());
    }

    @DataProvider
    Object[] getData(ITestContext context){
        return this.repository.findByDobBetween(Date.valueOf(context.getCurrentXmlTest().getParameter("dobFrom")), Date.valueOf(context.getCurrentXmlTest().getParameter("dobTo")))
                .stream()
                .limit(1)
                .toArray();
    }
